<template>
  <div>
    <ul class="nav navbar-nav on-style" v-if="($route.path.indexOf('/onboarding') !== -1 || onboarding) && !user.isVolunteer">
    ...
    <ul class="nav navbar-nav on-style" v-else-if="$route.path.indexOf('/onboarding') !== -1 || onboarding">
      <router-link to="/onboarding/profile" tag="li"><a class="profile-info">Basic Profile</a></router-link>
    </ul>
