...
<script>
<<<<<<< HEAD:src/components/Dashboard.vue
...
>>>>>>> master:src/views/DashboardView/index.vue
    },
    data() {
      const user = UserService.getUser() || {}
      SessionService.getCurrentSession(this, user)
      ...
      rejoinHelpSession() {
        ...
        if (path) {
          ...
          if (!user.isVolunteer) {
            ...
            const url = '/feedback/null/student/null/null'
            router.replace(url)
          }
          if (user.isVolunteer) {
            console.log('updating URL')
            const url = '/feedback/null/volunteer/null/null'
            router.replace(url)
          }
        }
      },
