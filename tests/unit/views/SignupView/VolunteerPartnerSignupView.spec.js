...
describe('VolunteerPartnerSignupView', () => {
  ...
  it('Renders step 2 after step 1 is complete', async () => {
    ...
    await wrapper
      ...
      .trigger('click')
    expect(
      wrapper.find('[data-testid="vp-signup-errors"]').exists()
    ).toBeFalsy()
    await nextTick()

    // Confirm step 2 renders
    expect(
      wrapper.find('[data-testid="vp-signup-step-2-title"]').isVisible()
    ).toBeTruthy()
    expect(
      wrapper.find('[data-testid="vp-signup-first-name"]').isVisible()
    ).toBeTruthy()
    expect(
      wrapper.find('[data-testid="vp-signup-last-name"]').isVisible()
    ).toBeTruthy()
    expect(
      wrapper.find('[data-testid="vp-signup-user-agreement"]').isVisible()
    ).toBeTruthy()
    expect(
      wrapper.find('[data-testid="vp-signup-complete-button"]').isVisible()
    ).toBeTruthy()
