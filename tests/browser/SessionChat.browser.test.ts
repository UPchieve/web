...
import { createVoiceChatMockStore, createVoiceChatMockSession } from './SessionChat.fixtures'
...
test('voice chat in-progress captions render correctly', async () => {
  ...
  await expect.element(page.getByText('This is a really long message from me')).toBeVisible()
  await expect.element(page.getByText('Hello, this is a completed message')).toBeVisible()
  await expect.element(page.getByText('This is a completed transcription')).toBeVisible()
  await expect.element(page.getByText('I am speaking right now...')).toBeVisible()
